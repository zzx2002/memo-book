...
│     ├─ views.ts                # 「今天 / 即将到来」归属判定与列表行日期展示
│     ├─ ai.ts                   # AI 整理：prompt 构造、响应解析、判重、token 估算（传输层可注入）
...
| `pnpm check:ui` | jsdom 中真实挂载 App，覆盖搜索、拖拽排序、重复任务、日期选择、回收站与撤销、内联重命名、键盘导航、提醒调度、提前量、导入导出、设置面板、多选批量、更新检查、AI 整理（103 项断言） |
...
「今天 / 即将到来」按**排期（开始日期）与截止日期一起**判定，
侧栏计数与列表筛选共用 `src/lib/views.ts` 里的同一套判断，避免两者算错不到一块去：

| 视图 | 归属条件（均要求未完成、未删除） |
| --- | --- |
| 今天 | 截止日期 ≤ 今天（今天到期或已逾期）**或** 排期 ≤ 今天（已开始，含刚排到今天） |
| 即将到来 | 不满足"今天"，但排期或截止日期在未来 |
| 收件箱 | 全部未完成（含两个日期都没设的） |

> 只设了排期、没设截止日期的任务同样计入「今天」，不会在三个视图里都消失。
> 列表行在有截止日期时显示截止日期，没有时显示 `开始 9月21日`（灰色），一眼能看出日期来源。
...
侧栏「AI 整理会议记录」→ 粘贴会议文字稿 → DeepSeek 抽取行动项 → 勾选确认 → 一键写入收件箱（可整体撤销）。

- **密钥安全**：API Key 存在 **Windows 凭据管理器**（Rust `keyring`），不写配置文件、不进仓库、前端也读不到；
  请求全部在 Rust 侧发出（`reqwest`），前端只负责构造 prompt 与解析结果
- **模型**：`deepseek-flash`（1M 上下文、支持 JSON 输出）；`response_format: json_object` + `temperature 0.2`
- **防幻觉设计**（LLM 做抽取时最容易出错的地方）：
  - prompt 明确要求"只抽取原文明确提到的行动项""严禁编造日期"，模糊时间必须留空并降低 confidence
  - 每条草稿都带 `sourceQuote`（原文逐字摘录），预览里直接显示，一眼能看出模型是否在编
  - 非 `YYYY-MM-DD` 的日期在解析阶段被丢弃；非法优先级回落为 `low`；文件夹不存在时在预览里标红
  - 与已有待办重复的草稿（标题 + 截止日期一致）默认**不勾选**
- **预览确认**：每条都可改标题、改优先级、换文件夹、调日期；显示置信度与「可能重复」标记；
  模型跳过的段落可展开看原因
- **成本可见**：预览头部显示 token 用量（输入 / 输出 / 命中缓存）
- **失败兜底**：JSON 模式偶发返回空内容（官方已知问题）→ 给出"请重试一次"；非 JSON、空结果都有可读提示；
  401/402/429/超时都映射为中文说明
- **可测试性**：prompt 构造与响应解析是纯函数，传输层可注入（`setAiTransport`），
  因此整条链路能在 jsdom 里用假响应完整跑通，不花真钱、可离线

### v0.6.1 视图归属修复

**问题**：只设了排期（开始日期）、没设截止日期的任务，在「今天」「即将到来」两个视图里都看不见，
侧栏计数也因此是 0 —— 明明列表里有一堆今天该做的事。

**根因**：侧栏计数与列表筛选**各写了一份判断逻辑**，且都只看截止日期。

**修复**：
- 抽出 `src/lib/views.ts` 作为唯一判定来源，计数与筛选共用，从结构上杜绝两者分叉
- 「今天」= 截止 ≤ 今天（今天到期/已逾期）**或** 排期 ≤ 今天（已开始）；「即将到来」= 日期在未来
- 列表行在没有截止日期时显示 `开始 9月21日`（灰色，hover 提示"排期开始日期"），此前排期完全不显示
- 视图副标题改为准确描述：「已开始、今天到期或已逾期的事」
